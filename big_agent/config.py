...
# 工作流配置参数
# ============================================================================
# 定义Big Agent工作流的基本运行参数
WORKFLOW_CONFIG = {
    "max_execution_time": 300,  # 最大执行时间（秒）- 防止长时间运行
    "retry_attempts": 3,        # 重试次数 - API调用失败时的重试机制
    "timeout": 60              # API超时时间（秒）- 单个API调用的超时设置
}
...
# 定义项目中各个功能模块的数据存储路径
PATHS = {
    "json_configs": "jsonFiles",      # JSON配置文件目录 - 存储指标计算配置

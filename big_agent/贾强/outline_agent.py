...
class OutlineGeneratorAgent:
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.deepseek.com"):
        ...
        self.industry_index = {
            "农业": [
                "农业-交易对手收入排名TOP3 (metric_id: 'agriculture_income_top3')",
                "农业-交易对手支出排名TOP3 (metric_id: 'agriculture_expense_top3')",
                "农业-总经营收入 (metric_id: 'agriculture_total_income')",
                "农业-总经营支出 (metric_id: 'agriculture_total_expense')"],
            "黑色金属": [
                "黑色金属 - 交易对手收入排名TOP3(metric_id: 'black_metal_income_top3')",
                "黑色金属 - 交易对手支出排名TOP3(metric_id: 'black_metal_expense_top3')",
                "黑色金属 - 总经营收入(metric_id: 'black_metal_total_income')",
                "黑色金属 - 总经营支出(metric_id: 'black_metal_total_expense')"]
        }

    def create_prompt(self, question: str, industry: str, sample_data: List[Dict]) -> str:
        """创建大纲生成提示"""
        ...
        # 行业指标
        industry_index = self.industry_index.get(industry, []);
    ...
    async def generate_outline(self, question: str, industry: str, sample_data: List[Dict[str, Any]]) -> ReportOutline:
        """异步生成大纲（修复版：自动补全缺失字段）"""
        prompt = self.create_prompt(question=question, industry=industry, sample_data=sample_data)
